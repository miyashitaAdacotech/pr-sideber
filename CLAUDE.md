...
- `github-tools.md` — gh CLI と MCP の使い分け (IMPORTANT: gh CLI は利用可能)
...
- 全て exit 2 でブロック型。失敗したら編集が拒否される（握り潰し不可）
- コマンド実行時: シークレット漏洩検出でブロック

### hooks / verify / CI の責務分担
- **hooks**: 編集時に即ブロック。fmt / lint / 型チェック / clippy / machete / audit
- **`/verify`**: hooks がカバーしない項目のみ。テスト / セキュリティ grep / diff チェック
- **CI**: 全項目を網羅。最終防衛ライン
- hooks で保証済みの項目を `/verify` や agent が再実行・言及しない
