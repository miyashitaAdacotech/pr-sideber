<script lang="ts">
	...
	const { tree, activeTabUrl, activeWorkspaceIssueNumber, onNavigate, onOpenWorkspace, onPin }: Props = $props();
</script>
	...
	<section class="epic-section">
		...
		{:else}
			{#each tree.roots as root (root.kind.type === "epic" ? `epic-${root.kind.number}` : root.kind.type === "issue" ? `issue-${root.kind.number}` : root.kind.type === "pullRequest" ? `pr-${root.kind.number}` : `session-${root.depth}`)}
				<TreeNode node={root} {activeTabUrl} {activeWorkspaceIssueNumber} {onNavigate} {onOpenWorkspace} {onPin} />
			{/each}
		{/if}
	</section>
{/if}
